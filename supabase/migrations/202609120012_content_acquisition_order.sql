-- Without an STT checkpoint, NULL and false are both non-matches.
-- Preserve the complete acquisition preference until there is a bound transcript.
begin;
  ...
  if action='heartbeat' then
    ...
    select coalesce(jsonb_agg(to_jsonb(q)),'[]') into result from (
      ...
      left join lateral (
        ...
        order by coalesce((a.sha256=tr.transcript->'reference'->'derivation'->>'audioSha256'
          and a.acquisition_version=coalesce(tr.transcript->'reference'->'derivation'->'audioCoverage'->>'version','complete-v1')
          and (a.coverage is not distinct from tr.transcript->'reference'->'derivation'->'audioCoverage')),false) desc,
