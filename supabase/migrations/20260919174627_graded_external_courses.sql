-- Preserve the existing Level 1 row and RPC signature for installed clients.
...
alter table public.external_course_catalog add constraint external_course_catalog_source_id_check
  check (source_id in ('voa-level1','voa-level2'));

create or replace function public.external_catalog_worker(action text, args jsonb default '{}'::jsonb) returns jsonb
language plpgsql security invoker set search_path = '' as $$
declare token uuid; snapshot jsonb; committed boolean; selected_source text; lesson_count integer;
begin
  ...
  if selected_source not in ('voa-level1','voa-level2')
  ...
  lesson_count := case selected_source when 'voa-level1' then 52 else 30 end;
  if action = 'read' then
    ...
    if jsonb_typeof(snapshot) is distinct from 'object' or octet_length(snapshot::text)>262144
      or snapshot->>'sourceId' is distinct from selected_source or snapshot->>'language' is distinct from 'en'
    ...
    if jsonb_array_length(snapshot->'entries') <> lesson_count or exists (
