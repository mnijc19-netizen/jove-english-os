...
alter table public.external_course_catalog add constraint external_course_catalog_source_id_check
  check (source_id in ('voa-level1','voa-level2','bbc-six-minute'));
...
begin
  ...
  if selected_source not in ('voa-level1','voa-level2','bbc-six-minute')
  ...
  if action = 'read' then
    ...
    if selected_source = 'bbc-six-minute' then
