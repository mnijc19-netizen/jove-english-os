-- English graded reading: five bounded first-party directories, titles and full lesson-page links only.
...
alter table public.external_course_catalog add constraint external_course_catalog_source_id_check
  check (source_id in ('voa-level1','voa-level2','bbc-six-minute','ja-tadoku','en-bc-reading'));
...
begin
  ...
  if selected_source not in ('voa-level1','voa-level2','bbc-six-minute','ja-tadoku','en-bc-reading')
  ...
  if action = 'read' then
    ...
    if selected_source = 'en-bc-reading' then
