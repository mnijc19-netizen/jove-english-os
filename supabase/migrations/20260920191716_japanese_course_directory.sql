-- Irodori maintenance: retain reviewed72 lesson identities; verify directory links without copying media.
-- Preserve existing data, leases, RLS and the metadata-only service capability.
...
alter table public.external_course_catalog add constraint external_course_catalog_source_id_check
  check (source_id in ('voa-level1','voa-level2','bbc-six-minute','ja-tadoku','en-bc-reading','ja-irodori'));
...
begin
  ...
  if selected_source not in ('voa-level1','voa-level2','bbc-six-minute','ja-tadoku','en-bc-reading','ja-irodori')
  ...
  if action = 'read' then
    ...
    if jsonb_typeof(snapshot) is distinct from 'object' or octet_length(snapshot::text)>262144
      or snapshot->>'sourceId' is distinct from selected_source or snapshot->>'language' is distinct from (case when selected_source in ('ja-tadoku','ja-irodori') then 'ja' else 'en' end)
    ...
    if selected_source = 'ja-irodori' then
      if entry_count <> 72 or exists (
        select 1 from jsonb_array_elements(snapshot->'entries') e
        where jsonb_typeof(e) is distinct from 'object'
          or jsonb_typeof(e->'course') is distinct from 'string'
          or coalesce(e->>'course','') not in ('starter','elementary01','elementary02','pre-intermediate')
          or jsonb_typeof(e->'position') is distinct from 'number'
          or coalesce(e->>'position','') !~ '^([1-9]|1[0-8])$'
          or (e->>'url') is distinct from ('https://www.irodori.jpf.go.jp/en/' || (e->>'course') || '/audio/lesson' || lpad(e->>'position',2,'0') || '.html')
      ) or (select count(distinct e->>'url') from jsonb_array_elements(snapshot->'entries') e)<>72 then
        raise exception 'Incomplete or rebound Japanese course directory' using errcode='22023';
      end if;
    elsif selected_source = 'en-bc-reading' then
      if entry_count < 5 or entry_count > 500 or exists (
        select 1 from jsonb_array_elements(snapshot->'entries') e
        where jsonb_typeof(e) is distinct from 'object'
          or jsonb_typeof(e->'id') is distinct from 'string' or length(e->>'id') not between 1 and 120
          or coalesce(e->>'id','') !~ '^[a-z0-9]+(-[a-z0-9]+)*$'
          or jsonb_typeof(e->'level') is distinct from 'string' or coalesce(e->>'level','') not in ('A1','A2','B1','B2','C1')
          or (e->>'url') is distinct from ('https://learnenglish.britishcouncil.org/free-resources/reading/' || lower(e->>'level') || '/' || (e->>'id'))
          or jsonb_typeof(e->'title') is distinct from 'string' or length(e->>'title') not between 1 and 160
          or (e->>'title') ~ '[<>[:cntrl:]]'
      ) or (select count(distinct e->>'url') from jsonb_array_elements(snapshot->'entries') e)<>entry_count
        or (select count(distinct e->>'level') from jsonb_array_elements(snapshot->'entries') e)<>5
        or exists (select 1 from jsonb_array_elements(snapshot->'entries') e group by e->>'level' having count(*)>100) then
        raise exception 'Invalid or incomplete English reading catalog' using errcode='22023';
      end if;
    elsif selected_source = 'ja-tadoku' then
