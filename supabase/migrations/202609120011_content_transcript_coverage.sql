-- A source digest alone does not identify the transcript's acquisition/timeline.
-- Keep existing rows and refuse rebinding an STT checkpoint to different coverage.
begin;
  ...
  if action='heartbeat' then
    ...
    select coalesce(jsonb_agg(to_jsonb(q)),'[]') into result from (
      ...
      left join lateral (
        ...
        order by (a.sha256=tr.transcript->'reference'->'derivation'->>'audioSha256'
          and a.acquisition_version=coalesce(tr.transcript->'reference'->'derivation'->'audioCoverage'->>'version','complete-v1')
          and (a.coverage is not distinct from tr.transcript->'reference'->'derivation'->'audioCoverage')) desc nulls last,
    ...
    if action='checkpoint' then
      if exists(select 1 from public.content_transcripts tr where tr.item_id=item_key and tr.revision=item.revision
          ...
          and (tr.transcript->'reference'->'derivation'->>'audioSha256'
            is distinct from args->'transcript'->'reference'->'derivation'->>'audioSha256'
          or tr.transcript->'reference'->'derivation'->'audioCoverage'
            is distinct from args->'transcript'->'reference'->'derivation'->'audioCoverage')) then
