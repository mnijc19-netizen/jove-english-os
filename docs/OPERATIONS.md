...
The bounded-prefix candidate also requires all seven forward migrations `202609120008` through `202609120014` **before** its runtime is activated. Keep budgets paused and the scheduler off during the transition; verify no active source lease, revoke Pages readiness approval and cancel active deployment attempts as required below. Do not replace applied migrations or reset item attempts to make deployment appear successful. Verify the14-version migration readback, then download and compare the actual deployed shared runtime/entries against the independently reviewed build; an ACTIVE version counter is insufficient.

Before/after migration, compare existing sync operations, service preferences/usage, content items/transcripts and audio metadata without exposing payloads. Existing audio must retain its object path/digest/state and receive `complete-v1` with null coverage; existing clips receive `source_acquisition_version='complete-v1'`; the new scheduling timestamp starts null. New request intents must not fabricate mappings for historical paid receipts. Unknown charges remain held, and an item with unmapped historical usage remains reconciliation-blocked. A new bounded test requires its own reviewed allowance/baseline and non-overwriting receipt, not replay of the old one-shot helper. These checks do not approve a lesson, settle a charge or start a schedule.

Before opening any content allowance, call the private `content_legacy_service_unmapped()` preflight and require false. This also detects an old `service_usage` row whose process/lease ended before its `content_usage` checkpoint: without a trustworthy item mapping, the **single-owner content boundary** is blocked, not merely one known item. No ledger status proves that missing mapping. A matching modern intent/request fingerprint, or a same-owner legacy content checkpoint with a non-null item, establishes the relevant binding; the existing per-item replay fence and atomic reservation still apply. A true/failed preflight requires independent reconciliation evidence; do not invent a mapping, alter a hold, reset attempts or change the request fingerprint to continue.
...
For diagnosis without generation, authenticated owners or a server-only job-token operator may POST exactly `{"action":"provider-status"}` to `content`. This does not execute refresh or change budgets/usage; it performs only the active OpenRouter route's bounded [current-key metadata GET](https://openrouter.ai/docs/api/api-reference/api-keys/get-current-api-key). The response excludes key labels/identities and reports `generationVerified:false`; accepted metadata is not proof of generation permission, account balance, available privacy-compatible models or a receipt for any historical attempt. Null means unknown/unlimited as reported, not zero. Non-2xx status is an observation, not a guessed billing outcome. Job `{}` remains refresh, and `{"probe":true}` remains a post-authentication `CONTENT_JOB_REQUEST` rejection. Keep spending paused while diagnosing; never clear old holds or switch to a paid retry merely because this read-only check succeeds.
