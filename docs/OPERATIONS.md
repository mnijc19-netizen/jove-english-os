...
The bounded-prefix candidate also requires all six forward migrations `202609120008` through `202609120013` **before** its runtime is activated. Keep budgets paused and the scheduler off during the transition; verify no active source lease, revoke Pages readiness approval and cancel active deployment attempts as required below. Do not replace applied migrations or reset item attempts to make deployment appear successful. Verify the13-version migration readback, then download and compare the actual deployed shared runtime/entries against the independently reviewed build; an ACTIVE version counter is insufficient.
