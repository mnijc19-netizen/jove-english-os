...
The Level1-only job is now enabled; its first operator dispatch reached the live handler and correctly observed the six-hour backoff. This is not a timer-triggered refresh or new-course delivery proof.52 beginner lessons remain a finite course sequence; broader graded sources, link-level alternatives and Japanese-specific courses are still required. `STATUS.md` records actual release and scheduling evidence.

### Graded directory expansion — September20 local candidate

The official [Level2 directory](https://learningenglish.voanews.com/p/6765.html) lists30 lessons and describes an intermediate course designed by American English teachers. The first, second and final publisher lesson pages were inspected; the directory also contains review units, which are not miscounted as new numbered lessons. These are archived lessons, not a current weekly publication feed. Both exact directory pages passed real bounded, DNS-pinned HTML retrieval and complete-position parsing without media, transcript or AI requests; that test uses fixture persistence, not production delivery.

The Level2 lessons include an animated Professor Bot teaching section alongside the acted conversations. Original Jove instructions explicitly use the main human conversation for listening/retell and tell learners not to use the character narration as their speaking model. This is publisher/course provenance with a known mixed-presentation limitation, not an all-segment human-voice or acoustic-quality certificate. Only page links and original prompts are stored; no video, audio, transcript, publisher quiz or answer is copied.

Each of the two allowlisted sources has an independent snapshot, lease and backoff. Omitted source IDs still mean Level1, preserving old request/response contracts and all three legacy material identities. Level2 IDs are separate; wrong-source readback, commits and leases fail closed. The browser starts independent bounded requests/transactions and visibly reports partial availability. The scheduler attempts both sources, keeps successful refreshes and reports aggregate failure if either request fails; it does not retry automatically.

Course positions advance independently from honest saved participation. Existing observed-input difficulty gates select between courses, and an already assigned task stays bound. Completion does not award skill evidence or promote proficiency. Difficulty numbers remain provisional editorial estimates, not CEFR calibration.82 archived lessons expand the reserve but do not alone complete long-term automatic supply, link-level recovery or broad advanced coverage. Deploy migration/runtime before exposing the new client/job requests; older clients continue receiving Level1 only.
...
## Executable worker and versioned acquisitions

Export runContentRefresh({adminClient,fetcher?,audioStore?,analyzeAudio?,transcribe?,budget?,ownerId?,now?,sources?,sourceIds?,rightsPolicies?,profile?,limits?,analyzerVersion?,transcriberVersion?,costCeilings?,transcriptOrigin?,audioAcquisition?,forcePoll?,signal?}) from src/server/content-worker.ts. Injection seams are trusted server/test code only. The candidate handler now explicitly selects `mpeg-prefix-v1`; the deployed246c1bc runtime still uses complete episodes. No hosted prefix activation is claimed.

Generic complete-acquisition bounds:all5allowlisted sources (maximum10),2items/source,6segments/item,25feed entries,90-second job (configurable maximum120s),64MiB episode. Prefix mode defaults to **one audio item across the entire run**, not one per source. Handler explicitly uses this cap,110s,1item/source and2candidate segments/item. Deferred sources/items are not failed or charged; later runs advance both sources and uninspected candidate intervals. First-two rejections cannot permanently exhaust a longer transcript.

The pinned fetcher accepts `audioPrefixBytes`, restricted to audio, byte zero, at most8MiB and no conditional request. Every redirect retains the prefix bound and repeats URL/DNS/TLS checks. A single-part206 must supply consistent known-total `Content-Range`/body length; a200 is usable only as a complete small response within the same cap. Duplicate/multipart/encoded/oversized/contradictory responses fail closed, with no full-download retry. The returned `byteCoverage` describes representation bytes only, never decoded time or inspected speech. [RFC9110 range semantics](https://www.rfc-editor.org/rfc/rfc9110.html#section-14) permit servers to ignore ranges; small successful probes do not guarantee all future enclosures support them. The candidate worker requests prefixes, persists measured complete MPEG frames before provider reservation, and filters transcript cues to that measured range. Small complete non-MPEG responses retain the old acquisition path; partial non-MPEG responses fail closed. Independent review, actual hosted runtime/provider evidence and final device acceptance still gate deployment.

The candidate `prepareContentMp3Prefix` now retains at most600 seconds of complete consecutive MPEG LayerIII frames from the single8MiB-bounded representation, records original byte offsets/received length/representation length/sample-count timing, and returns a new byte array. It rejects invalid coverage, metadata above1MiB, inconsistent version/rate/channels, interior junk, unavailable first-frame reservoir and invalid final partial headers. It excludes the bounded ID3 tag and the first Xing/Info/VBRI metadata frame rather than applying full-file totals to a prefix; the transformation is explicit, and gapless adjustment remains unapplied. The shared frame-header parser does not decode speech or certify phoneme/alignment quality. Relevant primary references are the [ID3v2.4 header/footer specification](https://id3.org/id3v2.4.0-structure), [LAME's Xing metadata implementation](https://github.com/lameproject/lame/blob/master/libmp3lame/VbrTag.c), and [FFmpeg's MP3 demuxer](https://github.com/FFmpeg/FFmpeg/blob/master/libavformat/mp3dec.c). No third-party implementation was copied into the project.

Actual September12 bounded2MiB probes after current rights/feed checks produced66.821224s for The Launch,89.991837s for LINUX Unplugged and90s for the selected Open Yap sample. The earlier raw5–25s frame-window passed Windows Chromium but failed Linux WebKit: AudioContext decoded it, while the media element reported0.000801503s and could not finish the seek/end test. Windows WebKit lacks Web Audio. Those failed observations are retained, not reclassified as passes.

The new `contentMp3PrefixWindow` builds a separate metadata-only Xing frame with this clip's actual frame count, byte length and100-entry variable-bitrate seek table, then copies the original encoded audio frames unchanged. `mpeg-frame-count-with-xing-v1` distinguishes the new immutable hash/container; no gapless/encoder/phoneme measurement is invented. Restored coverage is checked against frames, bounds, declared acquisition kind and the initial reservoir. `contentStoredAudioWindow` selects it only when explicit prefix coverage exists; legacy complete-episode clips retain their old bytes. Both audio-analysis and STT adapters support this explicit input contract. Migration008 extends clip timing/MIME compatibility;009 adds immutable acquisition versions;010 adds logical request bindings;011/012 fix exact transcript-coverage selection and SQL NULL fallback ordering. Existing rows/hashes, owner grants and RLS remain unchanged. All12 migrations are applied locally only; hosted remains at7.

With the source implementation (not the earlier prototype), all three5–25s clips decoded to finite nonzero PCM, reported matching durations and completed real play/seek/end in isolated Linux Chromium and WebKit: The Launch22.282449s, clip SHA256`793d2a8d7aaae7beab7ccedab37d66e0d301312ce4203b55f6993ff718b70ab4`; Open Yap22.296s,`9dac84e88fba0bd56a48b05a5512828be0f9dc21bda22532e6690d0f4a88815f`; LINUX Unplugged22.282449s,`bb8aebe72bd09bb92c7232e4378dd903701311f10162c324710ddabdf158d810`. The first two acquisitions completed through Linux's pinned transport. LINUX Unplugged's Linux audio download timed out; its decoder test used the independently acquired Windows-pinned artifact, verified by SHA256`1e560ffaae3c3d793382842de5b2aaeb676befab17c09cfa4987f028d1efcdcd` and mounted read-only. That does not close Linux network reliability. The opt-in `JOVE_CONTENT_PREFIX_DECODE_PROBE=1` now exercises the versioned container; Windows Web Audio limitations still fail rather than being hidden. No prefix was uploaded, paid-analyzed or made eligible. Immutable acquisition/cache migration, complete-cue coverage filtering, pipeline replay/retention/budget tests, independent review and real iPhone acceptance remain required before enabling the path.
...
3. Slice punctuation-bounded30–120s groups. Preserve real cue timing, reject gaps/unfinished tails; internal timing within a multi-sentence cue stays unknown, never interpolated.
   The slicer now accepts a trusted `audioCoverage` for a measured byte-zero artifact up to600s. It rejects invalid ranges and excludes any cue extending beyond the available end **before** sentence grouping and the segment limit. Pending unfinished sentences are discarded instead of completed using unheard punctuation; original cue indices, transcript references and fully covered lesson identities remain unchanged. This helper does not infer coverage from publisher text, approve a lesson, or mutate the full transcript/episode duration. The candidate worker passes actual restored/acquired coverage and caches the original transcript independently.
...
### Recovery, replay and retention boundaries — candidate September12

The hosted platform documents [256MB memory and2s CPU per request](https://supabase.com/docs/guides/functions/limits). An isolated instance of the installed Edge Runtime1.74.3, configured with256MB/1800ms soft CPU/2000ms hard CPU/150s wall limits, exercised the real parser/remuxer/hash/STT serialization and two120-second analysis serializations using synthetic8MiB MPEG input and an in-memory provider adapter. A single high-bitrate item completed in1000.71ms wall time; five high-bitrate items were cancelled by the CPU supervisor; five low-bitrate items completed in1645.21ms. These elapsed times are not measured CPU durations or a production capacity guarantee. No provider was contacted, and no human speech was approved. Bootstrap fetched public Node type metadata; the audio/model transport itself performed no external I/O. Initial harness bootstrap/tag failures are not application defects.

That native run also exposed a real full-prefix endpoint defect: total-sample division and repeated frame-duration addition can differ by sub-nanoseconds. Three failed-first8MiB MPEG/bitrate tests reproduce the refusal or inconsistent duration. The new prefix path validates the declared coverage strictly, normalizes only the verified endpoint for the legacy frame slicer and reports canonical sample-count duration; even a requested endpoint1ns beyond coverage still fails. All encoded speech frames and legacy full-acquisition timing/bytes are preserved.

Migration013 adds only `last_audio_work_at` and a service-only lease/revision-checked scheduling RPC. Prefix batches order least-recently-attempted sources first, using inventory priority for ties; even repeated runs at the same client clock advance. An attempt is recorded before expensive media work and remains recorded after source/storage/provider failure. A source that fails therefore yields its next turn; unstarted items retain attempts, retries, transcripts and billing records unchanged. This scheduling marker is neither learning evidence nor a provider receipt. Hosted capacity, continuing eligible supply and independent review remain separate open gates.

A later actual native full-worker probe used real local PostgreSQL/private Storage/atomic budgets with synthetic audio and provider responses. Separate cold isolates saved STT, resumed two then one remaining analyses without retranscribing, and processed a new STT+two-analysis item. All passed256MB/2s CPU supervision (830.31/750.69/489.03/1176.47ms wall time). Readback verified seven completed zero-cost fixture reservations, no uncertain usage, two transcripts and five ready clips. After the first source asset was deleted, its three clips remained downloadable with exact hashes. The initial harness's wrong two-segment assumption failed; draining all three existing segments corrected only that fixture expectation. Both scoped runs were cleaned up. This is full local orchestration evidence, not a paid-provider, actual human-content, peak-memory or hosted-capacity guarantee.

Separate no-credential native Deno public-source probes passed current rights checks, real pinned-TLS feeds and8MiB-prefix preparation for The Launch, Open Yap and LINUX Unplugged in7.98/5.31/22.35s wall time. LINUX's9,213,876-byte feed and audio both completed in this sample; the earlier Node timeout remains historical evidence. Generated5–25s clip hashes exactly match the earlier Linux browser-decoded clips. These probes made no database/Storage/eligibility writes and no AI calls. Combining real publisher latency with a live paid provider, sustained scheduled supply and production device acceptance remain unverified.

A later native replay of a different LINUX candidate exposed a filename-policy mismatch: ART19's already approved signed CDN URL contained `%20` in its MP3 basename. The narrow rule now permits encoded spaces only in that existing bounded basename, without decoding or admitting arbitrary escapes/query parameters/hosts/path segments. Failed-first URL and pinned-HTTP tests reproduce the rejection; the same reconstructed episode then returned206 under the original native TLS pinning and16KiB probe bound. Source cache/byte coverage and speech approval are unchanged. Redirect diagnostics redact transient signing fields. A successful fetch does not establish acceptable ads, accent, transcript alignment or a usable lesson.

`content_audio_assets` keys include `(item_id,revision,acquisition_version)`. Old `complete-v1` rows retain `episodes/{item}/{sha}` and null coverage; `mpeg-prefix-v1` uses `prefixes/{item}/{sha}` with strictly checked frame/byte/sample metadata. Both count against the existing combined256MiB source cache. A prefix does not replace a full recording, overwrite its digest, or redefine the original episode duration. Without an STT checkpoint, an existing complete acquisition is preferred. With one, pending selects the exact digest **and acquisition/coverage**, including when two versions share identical bytes. Changed STT digest or coverage cannot overwrite that checkpoint. Clips bind their source acquisition but remain usable after source-cache expiry.

Before any paid reservation, service-only `content_request_intent` binds the item revision, STT or segment scope, request fingerprint and audio digest under the current source lease. Changed acquisition/adapter fingerprints cannot turn an unresolved logical operation into a new paid request. An unchanged intent still must acquire the existing atomic budget reservation; the intent itself is **not** evidence of dispatch or cost. Deadline cancellation of this lookup cannot start a reservation. Legacy usage rows without such a binding conservatively block new dispatch on that item until independently reconciled; no historical charge, revision or interval is guessed. An intent can exist even if budget was subsequently denied, so changing that planned request also requires reconciliation rather than automatic rebinding.

An exact `content-provider-reconciliation-required` failure now checkpoints as quarantined, not generic awaiting-configuration. Before slicing the bounded ten-item pending window, the worker normalizes only legacy `awaiting-analysis` rows with that exact stored reason and the current process version through the existing lease/revision-fenced `finish-item` RPC. It checks cancellation before dispatch, appends the normal operational history entry and preserves reasons, saved media, intents and usage. Known blocked rows consume no episode/media slot; the visible reconciliation gate remains. Fresh pending revisions, changed processing versions and incomplete metadata still encounter the real request/budget gates; quarantine is not settlement or permission to replay. No SQL migration, larger network batch or automatic paid retry is introduced.

Migration014 additionally handles an interrupted **pre-intent** worker whose service reservation survived without its lease-fenced content checkpoint. The service-only `content_legacy_service_unmapped()` preflight checks active owner content-ledger rows against a matching request/fingerprint intent or a same-owner non-null-item content checkpoint. An unknown item closes the owner-wide content dispatch boundary before creating an intent, regardless of ledger status; no second reservation is authorized. A mapped old item retains the narrower per-item fence. A modern intent committed before reservation remains a trustworthy binding even if its later content checkpoint was interrupted; ordinary atomic reservation/replay checks remain mandatory. Holds and historical rows are never rewritten by this forward migration.

Expired prefix objects are marked inaccessible, removed by exact namespace/path, and then acknowledged in SQL; failed deletion returns the same paths. Cleanup preserves the complete acquisition, live clips, request bindings and unresolved usage evidence. Real local PostgreSQL tests cover JSONB STT restore, exact clip parent binding, identical-digest selection, legacy-hold refusal, service-only access and deletion retries. Their media, provider and Storage adapter are synthetic test fixtures, **not** screened human lessons or a hosted billing demonstration. Actual prefix upload/STT/analysis on the hosted runtime, native runtime limits, independent review and final device acceptance remain open.
...
Normal route uses the existing server OPENROUTER_API_KEY. Production explicitly sets `JOVE_CONTENT_AUDIO_MODEL=google/gemini-3.8-flash` following the owner's 2026-09-13 request; the unchanged source fallback is `gemini-2.5-flash` only when that setting is absent. Each job checks current catalog audio-input/structured-output capability. [Official OpenRouter docs](https://openrouter.ai/docs/guides/overview/multimodal/audio) support real base64 input_audio on chat completions for analysis; plaintext STT is not acoustic evidence. `JOVE_CONTENT_AUDIO_PROVIDER=gemini` selects the direct route and requires `GEMINI_API_KEY`. No third key is required normally; both routes inspect exactly persisted clip bytes. Configuration/catalog verification is not successful paid generation or approval of a human lesson; spending remains paused at this checkpoint.

HTTP failure checkpoints retain provider/stage/status plus optional closed-vocabulary error codes and up to four canonical parameter roots from structured error fields. Wire references: [OpenRouter errors](https://openrouter.ai/docs/api_reference/errors-and-debugging) and [Google field violations](https://docs.cloud.google.com/php/docs/reference/common-protos/latest/Rpc.BadRequest.FieldViolation). Error JSON is read only within16KiB/500ms and request cancellation; messages, raw bodies, headers, URLs and private field-path suffixes are never persisted. Missing, malformed or unsupported details remain absent. The observed HTTP failure is thrown before optional parsing so later worker cancellation cannot replace it with a timeout. None of this authorizes retries, settles an unknown charge or reconstructs previously discarded errors.
...
- A provider response can report a valid invoice while its transcript/lesson fails schema, timing or coverage validation. Preserve validated provider request ID, token usage and nullable reported cost in the private failure receipt; keep the operation uncertain and the lesson ineligible. Explicit reported zero differs from absent cost. Unknown/malformed receipts retain the original hold. Do not reset an old hold without its actual receipt. Enumerated internal service error codes survive worker persistence; raw messages, provider bodies and lookalike arbitrary error codes do not. This does not establish that a historical reservation reached the provider's HTTP endpoint.
