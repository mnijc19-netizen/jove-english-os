...
Actual September12 bounded2MiB probes after current rights/feed checks produced66.821224s for The Launch,89.991837s for LINUX Unplugged and90s for the selected Open Yap sample. Each5–25s test window decoded to nonzero PCM and reached the real media-element ended event in Windows Chromium. **The raw frame-window playback gate failed:** Windows WebKit lacks Web Audio, and an isolated Linux WebKit run decoded The Launch but reported only0.000801503s for its media-element duration and could not finish the seek/end test. A separate ignored remux prototype, adding a new Xing frame with clip-specific counts/length while preserving original audio frames, made that exact clip report22.282449s and reach ended in both Linux Chromium/WebKit. Two further Linux attempts stopped on bounded public-source timeouts before validating the remaining clips. The prototype is not integrated or approved, the strict opt-in `JOVE_CONTENT_PREFIX_DECODE_PROBE=1` test still detects the raw-window failure, and no prefix has been uploaded, analyzed for money or promoted to a lesson. A versioned container/timing solution, immutable acquisition migration, covered-caption filtering and complete pipeline/playback evidence remain required.
