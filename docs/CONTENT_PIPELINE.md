...
The next large-media acquisition increment adds an opt-in `audioPrefixBytes` to the existing pinned fetcher, restricted to audio, byte zero, at most8MiB and no conditional request. Every redirect retains the prefix bound and repeats URL/DNS/TLS checks. A single-part206 must supply consistent known-total `Content-Range`/body length; a200 is usable only as a complete small response within the same cap. Duplicate/multipart/encoded/oversized/contradictory responses fail closed, with no full-download retry. The returned `byteCoverage` describes representation bytes only, never decoded time or inspected speech. [RFC9110 range semantics](https://www.rfc-editor.org/rfc/rfc9110.html#section-14) permit servers to ignore ranges; small successful probes do not guarantee all future enclosures support them. **The worker does not yet request prefixes:** explicit partial-artifact persistence, frame/decoder coverage, versioned cache recovery and browser playback must land before enabling this path. Existing complete-episode contracts are unchanged.
...
The new `contentMp3PrefixWindow` builds a separate metadata-only Xing frame with this clip's actual frame count, byte length and100-entry variable-bitrate seek table, then copies the original encoded audio frames unchanged. `mpeg-frame-count-with-xing-v1` distinguishes the new immutable hash/container; no gapless/encoder/phoneme measurement is invented. Restored coverage is checked against frames, bounds, declared acquisition kind and the initial reservoir. `contentStoredAudioWindow` selects it only when explicit prefix coverage exists; legacy complete-episode clips retain their old bytes. Both audio-analysis and STT adapters now support that explicit input contract, but the production worker still supplies only complete episodes. Migration008 extends only clip timing/MIME compatibility, retaining old rows, hashes and RLS; it does not implement or activate versioned prefix asset acquisition.
